  ...
  for (let position of positions.slice(1)) {
    ...
    try {
      ...
      if (position.classification == Classification.BEST) {
        ...
        if (absoluteEvaluation >= 0 && !winningAnyways && position.move?.san && !position.move.san.includes("=")) {
          ...
          try {
            ...
            console.error("Error in brilliancy check:", error);
          }
          
          // If no sacrificed pieces, we can't have a brilliant move here
          ...
          try {
              ...
              for (let attacker of attackers) {
                for (let promotion of promotions) {
                  try {
                    ...
                    console.warn("Error testing capture:", e);
                  }
                }
            ...
            console.error("Error in capturable check:", error);
          }
        ...
        try {
          if (
            ...
            && topMove && secondTopMove
            && Math.abs((topMove.evaluation.value || 0) - (secondTopMove.evaluation.value || 0)) >= 150
          ) {
            // Check if the target square exists before checking hanging status
            const toSquare = position.move?.uci?.slice(2, 4);
            if (toSquare && position.move?.uci && 
                !isPieceHanging(lastPosition.fen, position.fen, toSquare)) {
              position.classification = Classification.GREAT;
            }
          }
        } catch (error) {
          console.error("Error in great move check:", error);
        }
      }
      ...
      position.classification = Classification.BOOK; // Default to safe classification
    }
  }
  ...
  for (let position of positions) {
    try {
      ...
      for (let line of position.topLines) {
        if (!line || !line.moveUCI || (line.evaluation?.type == "mate" && line.evaluation?.value == 0)) continue;
      ...
      console.error("Error generating SAN moves:", error);
    }
  }
